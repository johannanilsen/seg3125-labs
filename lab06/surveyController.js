...
module.exports = function(app){
    ...
    app.get('/johannassurvey', function(req, res){
        res.sendFile(__dirname+'/views/johannassurvey.html');
    });

    // when a user types SUBMIT in localhost:3000/johannassurvey 
    // the action.js code will POST, and what is sent in the POST
    // will be recuperated here, parsed and used to update the data files
    app.post('/johannassurvey', urlencodedParser, function(req, res){
        console.log(req.body);
        var json = req.body;
        for (var key in json){
            console.log(key + ": " + json[key]);
            // in the case of checkboxes, the user might check more than one
            if ((key === "side") && (json[key].length === 2)){
